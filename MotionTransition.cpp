	...
	const MotionInfo * prev_motion, const MotionInfo * next_motion, const Matrix4f & prev_motion_mat, float prev_begin_time )
{
	...
	blend_begin_time = prev_motion->blend_begin_time - prev_motion->begin_time;

	// 動作遷移のための動作ブレンドを行う終了時刻（前の動作の開始時刻 prev_begin_time を基準とするローカル時刻）
	blend_end_time = next_begin_time + (next_motion->blend_end_time - next_motion->begin_time);
	...
	float time, Posture * posture )
{
	...
	else
	{
		...
		// ブレンド比率（補間の重み）を計算
		blend_ratio = (local_time - blend_begin_time)/ (blend_end_time - blend_begin_time);
...
void  ComputeConnectionTransformation( const Matrix4f & prev_frame, const Matrix4f & next_frame, Matrix4f & trans_mat )
{
	...
	// 方法１
	trans_mat.invert(next_frame);
	trans_mat.mul(prev_frame, trans_mat);
	...
	// 座標変換を計算
	trans_mat.invert(next_frame2);
	trans_mat.mul(prev_frame2, trans_mat);
}
	...
	const Posture & next_pose, float next_ori, int base_segment, Matrix4f & trans_mat )
{
	...
	{
		// ※レポート課題
		prev_pos = prev_pose.root_pos;
		next_pos = next_pose.root_pos;
	}
	// 基準部位としてルート体節以外が指定された場合は、順運動学計算により、２つの姿勢の指定部位の位置を計算
	else
	{
		// ※レポート課題
		ForwardKinematics(prev_pose, segment_frames);
		segment_frames[base_segment ].get( &prev_pos);
		ForwardKinematics(next_pose, segment_frames);
		segment_frames[base_segment ].get(&next_pos);
	}

	// 前の姿勢の位置・向きを表す変換行列を計算
	// ※レポート課題
	prev_rot.rotY(prev_ori);
	prev_frame.set( prev_rot, prev_pos, 1.0f );

	// 次の姿勢の位置・向きを表す変換行列を計算
	// ※レポート課題
	next_rot.rotY(next_ori);
