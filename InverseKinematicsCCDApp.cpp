...
void  FindJointPath( const Skeleton * body, int base_joint_no, int ee_joint_no, vector< int > & joint_path, vector< int > & joint_path_signs )
{
	...
	{
		joint_path.push_back( joint_path2[joint_path2.size() - 1 - i] );
...
void  ApplyInverseKinematicsCCD( Posture & posture, int base_joint_no, int ee_joint_no, Point3f ee_joint_position )
{
	...
	for ( int i = 0; i < max_iteration; i++ )
	{
		...
		for ( int j = 0; j < joint_path.size(); j++ )
		{
			...
			// 現在の関節のローカル座標系を取得
			local_frame = segment_frames[segment->index];
			local_frame.m03 = joint_positions[joint->index].x;
			local_frame.m13 = joint_positions[joint->index].y;
			local_frame.m23 = joint_positions[joint->index].z;

			// ワールド座標系から現在の関節のローカル座標系への変換行列を計算
			trans_mat.invert(local_frame);

			// 現在の関節から末端関節への方向ベクトル（現在の関節のローカル座標系）を計算
			trans_mat.transform(ee_pos, &local_pos);
			ee_vec.normalize(local_pos);

			// 現在の関節から目標位置への方向ベクトル（現在の関節のローカル座標系）を計算
			trans_mat.transform(ee_joint_position, &local_pos);
			goal_vec.sub(local_pos, ee_vec);

			// 現在の関節の回転軸・回転角度（0～π）を計算
			rot_axis.cross(ee_vec, goal_vec);
			rot_angle = rot_axis.length();
			...
			rot.mul(rot, posture.joint_rotations[joint->index]);
