...
void  MotionInterpolationApp::AnimationWithInterpolation( float delta )
{
	...
	{
		// ※ レポート課題
		keytimes[ i ] = ( motions[0]->keytimes[i] - motions[0]->keytimes[0] ) * (1.0f - weight) + ( motions[1]->keytimes[i] - motions[1]->keytimes[0] ) * weight;
	}
	...
	for ( int i = 0; i < num_keyframes-1; i++ )
	{
		if ( ( local_time >= keytimes[ i ] ) && ( local_time <= keytimes[ i + 1 ] ) )
		{
			...
			// ※ レポート課題
			seg_time = (local_time - keytimes[i]) / (keytimes[i+1] - keytimes[i]);
	...
	for ( int i = 0; i < 2; i++ )
	{
		...
		// ※ レポート課題
		motion_time = motions[i]->keytimes[seg_no] + seg_time * ( motions[i]->keytimes[seg_no+1] - motions[i]->keytimes[seg_no]);
